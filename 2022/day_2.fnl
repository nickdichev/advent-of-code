...
(local parsed_input
       (let [str (reader.file :day_2_input)]
         (icollect [line (string.gmatch str "%u%s%u")]
           [(string.sub line 1 1) (string.sub line 3 3)])))

(local opponent-throw {:A :rock :B :paper :C :sissors})

(local my-throw {:X :rock :Y :paper :Z :sissors})

(local points-per-throw {:rock 1 :paper 2 :sissors 3})

(local points-per-outcome {:lose 0 :draw 3 :win 6})
