...
(fn split-input [contents]
  (let [len (string.len contents) split_at (// len 2)]
...
(fn has-overlap-3? [one two three]
  (let [one-contents (rucksack-contents one) two-contents (rucksack-contents two)]
    (accumulate [overlap false three-item (string.gmatch three "%a")]
      (if (and (. one-contents three-item) (. two-contents three-item)) three-item overlap)
    )
  )
)
...
(accumulate [sum 0 _ rucksacks (ipairs part1_input)]
  (let [(left right) (table.unpack rucksacks) overlap (has-overlap? left right)]
...
(accumulate [sum 0 _ rucksacks (ipairs part2_input)]
  (let [(one two three) (table.unpack rucksacks) overlap (has-overlap-3? one two three)]
    (if overlap (+ sum (char-code overlap)) sum)
  )
)
